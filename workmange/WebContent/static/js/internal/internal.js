...
function getSysLoginValidator() {
	...
	$.post("../internal/sysLoginValidator.do?sysUnionCode="+userName, {
				...
				if(data == "1"){
					var serviceurl = "../internal/enterprise.do?sysUnionCode="+userName;
					var s4 = "../4s/enterprise.do?sysUnionCode="+userName;
					if(is4sSystem(userName))
					{
						loginSubmit(s4);
					}else
					{
						loginSubmit(serviceurl);
					}
...
var is4sSystem = function(unionCodeVal)
{
	
	if(unionCodeReg.test(unionCodeVal)&&unionCodeVal.indexOf("94")==0){
		  return true;
	}
	return false; 
}
